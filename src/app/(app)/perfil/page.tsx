  ...
  return (
    <>
      ...
      <div className="space-y-3 px-4 pt-3">
        ...
        {perfil && (
          <>
            <div className="grid grid-cols-3 gap-3"><Indicador rotulo="Idade" valor={idade} unidade="anos" /><Indicador rotulo="Altura" valor={perfil.altura_cm ?? "-"} unidade="cm" /><Indicador rotulo="Peso" valor={perfil.peso_kg ?? "-"} unidade="kg" /></div>
            <Cartao>
              <Rotulo>Dados do treino</Rotulo>
              <dl className="mt-2 space-y-2 text-sm">
                {[["Objetivo", perfil.objetivo ?? "Não informado"], ["Nível", perfil.nivel], ["Início", fmtData(perfil.data_inicio)], ["Meta semanal", `${perfil.meta_semanal} treinos`], ["Restrições", perfil.restricoes ?? "Nenhuma"], ["Lesões", perfil.lesoes ?? "Nenhuma"]].map(([k, v]) => (
                  <div key={k as string} className="flex justify-between gap-3"><dt style={{ color: "var(--dim)" }}>{k}</dt><dd className="text-right">{v}</dd></div>
                ))}
              </dl>
            </Cartao>
          </>

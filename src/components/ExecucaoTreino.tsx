...
export default function ExecucaoTreino({
  ...
  // Abre o treino local e guarda a ficha para funcionar sem internet
  useEffect(() => {
  ...
  const finalizar = async () => {
    setSalvando(true);
    const resultado = await finalizarTreinoLocal(
      localId.current, Math.max(1, Math.round(decorrido / 60)), observacoes
    );
    ...
    <div className="pb-40">
      <header className="sticky top-0 z-30 px-4 pb-3 pt-3"
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setAlerta(!alerta)} aria-label="Alerta sonoro"
            style={{ color: alerta ? "var(--marca)" : "var(--fraco)" }}>
            <Bell size={18} />
          </button>
        </div>
        <div className="mt-3"><Barra valor={(registradas / totalSeries) * 100} /></div>
        <div className="mt-1 flex justify-between text-xs" style={{ color: "var(--fraco)" }}>
          <span>{registradas} de {totalSeries} séries</span>
          <span>{indice + 1}/{itens.length} exercícios</span>
        </div>
      </header>
      ...
      <div className="px-4 pt-4">
        <div className="sem-barra flex gap-2 overflow-x-auto pb-2">
          {itens.map((i, n) => {
            ...
            return (
              <button key={i.id} onClick={() => setIndice(n)}
                className="numero shrink-0 rounded-lg px-3 py-2 text-xs"
                style={{
                  background: n === indice ? "var(--marca)" : pronto ? "var(--ok)" : "var(--superficie-2)",
                  color: n === indice || pronto ? "#fff" : "var(--dim)",
        ...
        <Cartao className="mt-2">
          <div className="flex items-start justify-between">
            <div>
              <Rotulo>{exercicio?.categorias_musculares?.nome} · {exercicio?.equipamentos?.nome}</Rotulo>
              <Titulo tamanho={22}>{exercicio?.nome}</Titulo>
            </div>
            <div className="text-right">
              <Rotulo>Série</Rotulo>
              <span className="numero text-xl">{Math.min(serieAtual, item.series)}/{item.series}</span>
            </div>
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            <Etiqueta>{item.rep_min}-{item.rep_max} reps</Etiqueta>
            <Etiqueta>Cadência {item.cadencia}</Etiqueta>
          ...
          {item.observacoes && (
            <p className="mt-3 rounded-xl p-3 text-xs" style={{ background: "var(--superficie-2)", color: "var(--dim)" }}>
              {item.observacoes}
            </p>
          )}
        </Cartao>
        ...
        <Cartao className="mt-3">
          <div className="grid grid-cols-2 gap-3">
            <ContadorNumerico rotulo="Carga (kg)" valor={carga} passo={2.5} aoMudar={setCarga} />
            <ContadorNumerico rotulo="Repetições" valor={reps} passo={1} aoMudar={setReps} />
          </div>

          <div className="mt-3">
            <div className="flex justify-between">
              <Rotulo>Percepção de esforço</Rotulo>
              <span className="numero text-xs">{pse}/10</span>
            </div>
            <div className="mt-2 flex gap-1">
              {Array.from({ length: 10 }).map((_, i) => (
                <button key={i} onClick={() => setPse(i + 1)} aria-label={`Esforço ${i + 1}`}
                  className="h-7 flex-1 rounded"
                  style={{
                    background: i < pse ? (i >= 8 ? "#D62828" : i >= 6 ? "#F4C20D" : "#16A34A") : "var(--superficie-2)",
                    border: "1px solid var(--linha)",
                  }} />
              ))}
            </div>
          ...
          {superaAnterior && (
            <div className="mt-3 flex items-center gap-2 rounded-xl px-3 py-2 text-xs"
              style={{ background: "#F4C20D22", color: "#F4C20D" }}>
              <Trophy size={14} /> Acima da sua melhor carga registrada ({ultima.carga} kg)
            </div>
          )}
          {progressao && (
            <div className="mt-2 rounded-xl px-3 py-2 text-xs" style={{ background: "var(--superficie-2)", color: "var(--dim)" }}>
              {progressao.motivo}. Sugestão para o próximo treino: {progressao.cargaSugerida} kg.
            </div>
          )}

          <button onClick={concluirSerie} disabled={serieAtual > item.series}
            className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl py-4 font-semibold text-white disabled:opacity-40"
            style={{ background: "var(--marca)" }}>
            {serieAtual > item.series ? "Exercício concluído" : `Concluir série ${serieAtual}`} <Check size={17} />
          </button>

          <div className="mt-2 grid grid-cols-3 gap-2">
            <BotaoSecundario onClick={() => setIndice(Math.min(itens.length - 1, indice + 1))}>
              <SkipForward size={14} /> Pular
            </BotaoSecundario>
            <BotaoSecundario onClick={() => alert("Substituição de exercício entra junto com o catálogo de similares.")}>
              <Repeat size={14} /> Trocar
            </BotaoSecundario>
            <BotaoSecundario onClick={() => setDescanso(item.descanso_seg ?? 60)}>
            ...
            {Array.from({ length: item.series }).map((_, i) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between rounded-lg px-3 py-2"
                  style={{ background: f ? "var(--superficie-2)" : "transparent", border: f ? "1px solid transparent" : "1px solid var(--linha)" }}>
                  <span className="display text-sm" style={{ color: "var(--dim)", letterSpacing: "0.08em" }}>
                    Série {i + 1}{i < (item.series_aquecimento ?? 0) ? " · aquecimento" : ""}
                  </span>
                  <span className="numero text-xs">
                    {f ? `${f.carga_kg} kg × ${f.repeticoes} · PSE ${f.pse}` : "pendente"}
                  </span>
                </div>
              );
            })}
        ...
        <Cartao className="mt-3">
          <Rotulo>Observação do treino</Rotulo>
        ...
        <button onClick={() => setFechando(true)}
          className="mt-3 w-full rounded-xl py-4 font-semibold text-white" style={{ background: "var(--ok)" }}>
          Finalizar treino
        </button>
        <p className="mt-2 text-center text-xs" style={{ color: "var(--fraco)" }}>
          <Save size={11} className="mr-1 inline" />
          Cada série é gravada no aparelho antes de subir para a nuvem
        </p>
      </div>
      ...
      {descanso !== null && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center px-8"
          ...
          <div className="numero" style={{ fontSize: 76, color: descanso <= 5 ? "var(--marca)" : "var(--texto)" }}>
            {fmtHora(descanso)}
          </div>
          <div className="mt-4 w-full max-w-xs">
            <Barra valor={100 - (descanso / (item.descanso_seg || 60)) * 100} altura={6} />
          </div>
          <div className="mt-6 flex gap-2">
            <BotaoSecundario onClick={() => setDescanso(Math.max(0, descanso - 15))}><Minus size={15} /> 15s</BotaoSecundario>
            <BotaoSecundario onClick={() => setPausado(!pausado)}>
              {pausado ? <Play size={15} /> : <Pause size={15} />} {pausado ? "Retomar" : "Pausar"}
            </BotaoSecundario>
            <BotaoSecundario onClick={() => setDescanso(descanso + 15)}><Plus size={15} /> 15s</BotaoSecundario>
          </div>
          <button onClick={() => setDescanso(null)}
            className="mt-6 w-full max-w-xs rounded-xl py-3 font-semibold text-white" style={{ background: "var(--marca)" }}>
            Pular descanso
          </button>
        </div>
      )}
      ...
      {fechando && (
        <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: "rgba(0,0,0,.55)" }}
          ...
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-t-3xl p-4"
            style={{ background: "var(--superficie)" }}>
            <Titulo tamanho={18}>Finalizar treino</Titulo>
            <div className="mt-3 grid grid-cols-3 gap-3">
              <Indicador rotulo="Duração" valor={Math.round(decorrido / 60)} unidade="min" />
              <Indicador rotulo="Séries" valor={registradas} />
              <Indicador rotulo="Volume" valor={Math.round(Object.values(feitas).reduce((t, f) => t + f.carga_kg * f.repeticoes, 0))} unidade="kg" />
            </div>
            {pendentes > 0 && (
              <p className="mt-3 flex items-center gap-2 text-xs" style={{ color: "var(--alerta)" }}>
                <CloudOff size={14} /> {pendentes} séries aguardando internet. Elas sobem sozinhas assim que a rede voltar.
              </p>
            )}
            <button onClick={finalizar} disabled={registradas === 0 || salvando}
              className="mt-4 w-full rounded-xl py-4 font-semibold text-white disabled:opacity-40"
              style={{ background: "var(--ok)" }}>
              {salvando ? "Salvando" : "Salvar treino"}
            </button>
            <button onClick={() => setFechando(false)}
              className="mt-2 w-full rounded-xl py-3 text-sm" style={{ border: "1px solid var(--linha)" }}>
              Continuar treinando
            </button>
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <div className="mt-1 flex items-center gap-2">
        <button onClick={() => aoMudar(Math.max(0, valor - passo))} aria-label={`Diminuir ${rotulo}`}
          className="rounded-lg p-2" style={{ background: "var(--superficie-2)" }}><Minus size={16} /></button>
        <input value={valor} inputMode="decimal"
          onChange={(e) => aoMudar(Number(e.target.value.replace(",", ".")) || 0)}
          className="numero w-full rounded-lg py-2 text-center text-xl outline-none"
          style={{ background: "var(--superficie-2)", border: "1px solid var(--linha)", color: "var(--texto)" }} />
        <button onClick={() => aoMudar(valor + passo)} aria-label={`Aumentar ${rotulo}`}
          className="rounded-lg p-2" style={{ background: "var(--superficie-2)" }}><Plus size={16} /></button>
      </div>
    </div>
  );
}
...
function BotaoSecundario({ children, onClick }: { children: React.ReactNode; onClick: () => void }) {
  return (
    <button onClick={onClick}
      ...
      style={{ border: "1px solid var(--linha)", color: "var(--texto)" }}>
      {children}
    </button>
  );
}
...
function tocarAviso() {
  try {
    ...
    // silêncio quando o navegador bloqueia áudio sem interação
  }
}

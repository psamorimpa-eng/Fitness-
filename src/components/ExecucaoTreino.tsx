...
import {
  iniciarTreinoLocal, registrarSerie, finalizarTreinoLocal, guardarFicha, sincronizar,
  carregarTreinoLocalEmAndamento, type SerieLocal,
} from "@/lib/offline";
...
type EstadoSync = "preparando" | "salvando" | "salvo" | "offline" | "erro";

function chaveRegistro(exercicioId: string, serie: number) {
  return `${exercicioId}-${serie}`;
}

function montarRegistros(series: Array<{ exercicio_id: string; numero_serie: number; carga_kg: number; repeticoes: number; pse: number | null }> = []) {
  const resultado: Record<string, Registro> = {};
  series.forEach((s) => {
    resultado[chaveRegistro(s.exercicio_id, s.numero_serie)] = {
      exercicio_id: s.exercicio_id,
      numero_serie: s.numero_serie,
      carga_kg: Number(s.carga_kg),
...
export default function ExecucaoTreino({
  alunoId, fichaId, fichaNome, divisao, itens, ultimasCargas, treinoEmAndamento,
}: {
  alunoId: string;
  fichaId: string | null;
  ...
  treinoEmAndamento?: TreinoAberto;
}) {
  const router = useRouter();
  const localId = useRef(treinoEmAndamento?.local_id ?? `${alunoId}-${divisao.id}-${Date.now()}`);
  const inicioIso = useRef(treinoEmAndamento?.inicio_em ?? new Date().toISOString());
  const inicio = useRef(new Date(inicioIso.current).getTime());

  const registrosServidor = useMemo(
    () => montarRegistros(treinoEmAndamento?.series_realizadas ?? []),
    [treinoEmAndamento]
  );
  const [indice, setIndice] = useState(0);
  const [feitas, setFeitas] = useState<Record<string, Registro>>(registrosServidor);
  const [decorrido, setDecorrido] = useState(() => Math.max(0, Math.floor((Date.now() - inicio.current) / 1000)));
  const [descanso, setDescanso] = useState<number | null>(null);
  const [pausado, setPausado] = useState(false);
  const [alerta, setAlerta] = useState(true);
  const [observacoes, setObservacoes] = useState(treinoEmAndamento?.observacoes ?? "");
  const [pendentes, setPendentes] = useState(0);
  const [fechando, setFechando] = useState(false);
  const [salvando, setSalvando] = useState(false);
  const [gravandoSerie, setGravandoSerie] = useState(false);
  const [estadoSync, setEstadoSync] = useState<EstadoSync>("preparando");
  ...
  const planoSnapshot = useMemo(() => itens.map((i) => ({
    exercicio_id: i.exercicio_id,
    nome: i.exercicios?.nome ?? "Exercício",
    ordem: i.ordem,
    series: i.series,
    rep_min: i.rep_min,
    rep_max: i.rep_max,
  ...
  const dadosTreinoLocal = (obs: string) => ({
    local_id: localId.current,
    aluno_id: alunoId,
    ficha_id: fichaId,
    divisao_id: divisao.id,
    data: inicioIso.current.slice(0, 10),
    inicio_em: inicioIso.current,
    fim_em: null,
    duracao_min: null,
    observacoes: obs,
    ficha_nome_snapshot: treinoEmAndamento?.ficha_nome_snapshot ?? fichaNome,
    divisao_codigo_snapshot: treinoEmAndamento?.divisao_codigo_snapshot ?? divisao.codigo,
    divisao_nome_snapshot: treinoEmAndamento?.divisao_nome_snapshot ?? divisao.nome,
    plano_snapshot: (treinoEmAndamento?.plano_snapshot as unknown[] | undefined) ?? planoSnapshot,
  });
  ...
  useEffect(() => {
    ...
    async function preparar() {
      try {
        const local = await carregarTreinoLocalEmAndamento(alunoId, divisao.id);
        if (local) {
          const servidorInicio = treinoEmAndamento?.inicio_em ?? "";
          const usarLocal = !treinoEmAndamento || local.treino.inicio_em >= servidorInicio;
          if (usarLocal) {
            localId.current = local.treino.local_id;
            inicioIso.current = local.treino.inicio_em;
            inicio.current = new Date(local.treino.inicio_em).getTime();
            setObservacoes(local.treino.observacoes ?? "");
            setFeitas((atuais) => ({ ...atuais, ...montarRegistros(local.series) }));
          } else if (local.treino.local_id === localId.current) {
            setFeitas((atuais) => ({ ...atuais, ...montarRegistros(local.series) }));
          }
        }

        if (cancelado) return;
        setEstadoSync(navigator.onLine ? "salvando" : "offline");
        await iniciarTreinoLocal(dadosTreinoLocal(observacoes));
        await guardarFicha(divisao.id, { divisao, itens, fichaId, fichaNome });
        const resultado = await sincronizar();
        if (cancelado) return;
        setPendentes(resultado.pendentes);
        setEstadoSync(resultado.erro ? "erro" : resultado.pendentes ? "offline" : "salvo");
        setPreparado(true);
      } catch {
        if (!cancelado) {
          setEstadoSync("erro");
  ...
  useEffect(() => {
    const t = setInterval(() => setDecorrido(Math.max(0, Math.floor((Date.now() - inicio.current) / 1000))), 1000);
  ...
  const serieAtual = useMemo(() => {
    for (let s = 1; s <= (item?.series ?? 0); s++) {
      if (!feitas[chaveRegistro(item.exercicio_id, s)]) return s;
    }
    return (item?.series ?? 0) + 1;
  }, [feitas, item]);

  const concluirSerie = async () => {
    if (!item || gravandoSerie || serieAtual > item.series) return;
    setGravandoSerie(true);
    const chave = chaveRegistro(item.exercicio_id, serieAtual);
    ...
    try {
      const serieLocal: Omit<SerieLocal, "id" | "enviada"> = {
        ...
        exercicio_nome_snapshot: item.exercicios?.nome ?? null,
        ordem_exercicio: item.ordem,
        series_planejadas: item.series,
        rep_min_planejada: item.rep_min,
        rep_max_planejada: item.rep_max,
        carga_planejada: item.carga_sugerida,
        descanso_planejado: item.descanso_seg,
        observacoes: item.observacoes,
        status_serie: "concluida",
      };
      await registrarSerie(serieLocal);
      const resultado = await sincronizar();
      setPendentes(resultado.pendentes);
      setEstadoSync(resultado.erro ? "erro" : resultado.pendentes ? "offline" : "salvo");
    } catch {
  ...
  const finalizar = async () => {
    ...
    setEstadoSync(resultado.pendentes ? "offline" : "salvo");
  ...
  return (
    <div className="pb-40">
      <header className="sticky top-0 z-30 px-4 pb-3 pt-3" style={{ background: "var(--bg)", borderBottom: "1px solid var(--linha)" }}>
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            <Rotulo>Treino {divisao.codigo}</Rotulo>
            <div className="numero text-sm">{fmtHora(decorrido)}</div>
            <StatusSync estado={estadoSync} />
          </div>
          <button onClick={() => setAlerta(!alerta)} aria-label="Alerta sonoro" style={{ color: alerta ? "var(--marca)" : "var(--fraco)" }}><Bell size={18} /></button>
        </div>
        <div className="mt-3"><Barra valor={totalSeries ? (registradas / totalSeries) * 100 : 0} /></div>
      ...
      <div className="px-4 pt-4">
        {treinoEmAndamento && (
          <div className="mb-3 rounded-xl px-3 py-2 text-xs" style={{ background: "rgba(22,163,74,.12)", color: "var(--ok)" }}>
            Treino retomado. Suas séries anteriores foram restauradas.
          </div>
        )}

        <div className="sem-barra flex gap-2 overflow-x-auto pb-2">
          {itens.map((i, n) => {
            const pronto = Array.from({ length: i.series }).every((_, s) => feitas[chaveRegistro(i.exercicio_id, s + 1)]);
            return (
        ...
        <Cartao className="mt-3">
          ...
          <button onClick={concluirSerie} disabled={!preparado || gravandoSerie || serieAtual > item.series} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl py-4 font-semibold text-white disabled:opacity-40" style={{ background: "var(--marca)" }}>
            {gravandoSerie ? <><Loader2 size={17} className="animate-spin" /> Salvando série</> : serieAtual > item.series ? "Exercício concluído" : <>Concluir série {serieAtual} <Check size={17} /></>}
          </button>
        ...
        <Cartao className="mt-3">
          ...
          <div className="mt-2 space-y-1">
            {Array.from({ length: item.series }).map((_, i) => {
              const f = feitas[chaveRegistro(item.exercicio_id, i + 1)];
        ...
        <p className="mt-2 text-center text-xs" style={{ color: "var(--fraco)" }}>
          <Save size={11} className="mr-1 inline" /> Cada série é salva no aparelho e sincronizada com sua conta
        </p>
      </div>
      ...
      {fechando && (
        <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: "rgba(0,0,0,.55)" }} onClick={() => setFechando(false)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-t-3xl p-4" style={{ background: "var(--superficie)" }}>
            ...
            {pendentes > 0 && <p className="mt-3 flex items-center gap-2 text-xs" style={{ color: "var(--alerta)" }}><CloudOff size={14} /> {pendentes} alterações aguardando internet. Elas serão sincronizadas quando a conexão voltar.</p>}
...
function StatusSync({ estado }: { estado: EstadoSync }) {
  const config = {
    preparando: { texto: "Preparando…", icone: <Loader2 size={10} className="animate-spin" /> },
    salvando: { texto: "Salvando…", icone: <Loader2 size={10} className="animate-spin" /> },
    salvo: { texto: "Salvo", icone: <Cloud size={10} /> },
    offline: { texto: "Salvo no aparelho · aguardando internet", icone: <CloudOff size={10} /> },
    erro: { texto: "Erro ao salvar · tentando novamente", icone: <AlertTriangle size={10} /> },
  }[estado];
  return <div className="mt-0.5 flex items-center justify-center gap-1 text-[10px]" style={{ color: estado === "erro" ? "var(--marca)" : "var(--fraco)" }}>{config.icone}{config.texto}</div>;
}
